...
from gamestore.models import (
    ...
    Order, FullOrder, CreateOrder, OrderGame
)

from gamestore.db_handler import (
    order_table as db,
    order_game_table as rel_table,
    Cursor
)
    ...
    with Cursor() as c:
        c.execute("""
            ...
            from
                "order" as o
            inner join
                order_game as og on og.order_id = o.id
            inner join
                game as g on og.game_id = g.id
            where
                o.id = :order_id""",
            {'order_id': order_id}
        )
